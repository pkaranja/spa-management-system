...
public class KycResource {
    ...
    @GetMapping
    public ResponseEntity<List<KycDTO>> getAllCustomers() {
        return ResponseEntity.ok(kycService.findAll());
    }

    @GetMapping("/{id}")
    public ResponseEntity<KycDTO> getCustomer(@PathVariable(name = "id") final UUID id) {
        return ResponseEntity.ok(kycService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<UUID> createCustomer(@RequestBody @Valid final KycDTO kycDTO) {
